...
Release: **v3.3.0** · Image: **`ghcr.io/mckenna654/nuvioexporter:3.3.0`**
...
2. Change the image to `ghcr.io/mckenna654/nuvioexporter:3.3.0` and the container name to `NuvioExporter`.
...
4. Apply the change and open `http://YOUR-UNRAID-IP:7088/api/health`. It should report `NuvioExporter` version `3.3.0` with status `ok`.
...
1. Download [`unraid-template.xml`](https://github.com/mckenna654/NuvioExporter/releases/download/v3.3.0/unraid-template.xml).
...
The template pins 3.3.0 and includes the WebUI, icon, support link, port, and appdata mapping.
...
| --- | --- |
| Name | `NuvioExporter` |
| Repository | `ghcr.io/mckenna654/nuvioexporter:3.3.0` |
| Network Type | `Bridge` |
| Privileged | `Off` |
...
Download [`docker-compose.release.yml`](https://github.com/mckenna654/NuvioExporter/releases/download/v3.3.0/docker-compose.release.yml). It binds to localhost by default. Set a trusted LAN address before starting it when another device must reach NuvioExporter:
...
6. For Remux, enter its server address, an administrator API key, and a stable setup name. Preview before import. Reusing the name updates the marked setup without deleting unrelated collections. The optional wipe mode previews the exact number of collection objects it will replace and requires typing `WIPE COLLECTIONS`; back up Remux first.
...
Never post widget exports, configured addon URLs, Remux API keys, the appdata database, or unsanitized screenshots publicly. Share the [project](https://github.com/mckenna654/NuvioExporter) or [release](https://github.com/mckenna654/NuvioExporter/releases/tag/v3.3.0).
